    ...
    <main className="relative flex h-screen w-screen flex-col overflow-hidden bg-stone-950 px-16 py-10">
      ...
      <div className="relative z-10 flex flex-1 flex-col items-center justify-center space-y-12 pb-12">
        ...
        <div className="grid w-full max-w-5xl grid-cols-2 gap-12">
          ...
          >
            {/* Ultra-Simple Minimalist SVG Illustration for Kiosk Machine (Animated Internal Screen) */}
            <div className="relative flex-1 flex items-center justify-center w-full">
               <svg width="200" height="200" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg" className="relative z-10 opacity-90">
                 ...
                 {/* Animated Screen Background Glow */}
                 <motion.rect 
                   animate={{ opacity: [0.05, 0.2, 0.05] }}
                   transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
                   x="45" y="25" width="110" height="120" rx="8" fill="#fbbf24" 
                 />
                 
                 {/* Animated UI Elements (Simulating active selection) */}
                 <rect x="55" y="35" width="90" height="30" rx="6" stroke="#fbbf24" strokeWidth="1" strokeOpacity="0.5" />
                 <motion.rect 
                   animate={{ opacity: [0.4, 0.9, 0.4] }}
                   transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
                   x="55" y="35" width="90" height="30" rx="6" fill="#fbbf24" 
                 />
                 ...
                 {/* Loading/Processing indicator on screen */}
                 <motion.circle 
                   animate={{ scale: [1, 1.2, 1], opacity: [0.5, 1, 0.5] }}
                   transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut", delay: 0.5 }}
                   cx="100" cy="130" r="4" fill="#fbbf24" 
                 />
               </svg>
            </div>
            
            <div className="text-center space-y-3">
              <h2 className="font-serif text-[2.5rem] font-normal text-amber-400">Layar Mesin</h2>
          ...
          <motion.button
            ...
            className="relative flex h-[500px] flex-col items-center justify-between overflow-hidden rounded-[2rem] bg-stone-900/60 p-10 backdrop-blur-xl border border-white/10 active:border-amber-500/50 transition-colors"
          >
            {/* Ultra-Simple Minimalist SVG Illustration for Phone (Animated Scanning) */}
            <div className="relative flex-1 flex items-center justify-center w-full">
               <svg width="200" height="200" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg" className="relative z-10 opacity-90">
                 {/* Gradient for Laser Light */}
                 <defs>
                 ...
                 {/* Screen Area (Dimly lit) */}
                 <rect x="70" y="30" width="60" height="140" rx="8" fill="#f59e0b" fillOpacity="0.05" />
                 
                 {/* Minimal QR Scanning Target */}
                 <rect x="80" y="75" width="40" height="40" rx="4" stroke="#fbbf24" strokeWidth="1.5" strokeOpacity="0.5" strokeDasharray="4 4" />
                 
                 {/* Animated Scanning Laser Line */}
                 <motion.line 
                   animate={{ y1: [75, 115, 75], y2: [75, 115, 75] }}
                   transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
                   x1="75" x2="125" stroke="#fbbf24" strokeWidth="2" strokeOpacity="0.9"
                 />
                 {/* Animated Scanning Light Sweep (Follows the line) */}
                 <motion.rect 
                   animate={{ y: [55, 95, 55] }}
                   transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
                   x="75" width="50" height="20" fill="url(#laserGlow)" 
                 />
               </svg>
            </div>
            
            <div className="text-center space-y-3">
              <h2 className="font-serif text-[2.5rem] font-normal text-amber-400">Smartphone</h2>
              <p className="text-lg font-light text-stone-300 leading-relaxed">
                Scan QR Code untuk melanjutkan di perangkat Anda.
              </p>
            </div>
          </motion.button>
