...
export default function ResultClient({ consultationId, menuId, explanation, recipe }: AIResultProps) {
  ...
  const handleBrew = () => {
    router.push(`/brewing?menu=${menuId}&consultationId=${consultationId}`);
  };
  ...
  return (
    <main className="relative flex min-h-screen w-screen flex-col overflow-hidden bg-stone-950 px-16 py-10">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden">
        <Image src="/hero-bg-traditional.png" alt="Background" fill className="object-cover object-center scale-105 blur-xs" priority />
      ...
      <div className="relative z-10 flex flex-1 flex-row items-center justify-center w-full gap-12 px-8">
        ...
        <motion.div 
          ...
          className="w-115 shrink-0 relative flex flex-col overflow-hidden rounded-[3rem] bg-linear-to-b from-stone-900/90 to-stone-900/50 backdrop-blur-3xl border-2 border-white/5 shadow-[0_20px_50px_rgba(0,0,0,0.6)]"
        >
          {/* Full Bleed Image Container */}
          <div className="relative w-full h-80 bg-stone-950 shrink-0 overflow-hidden">
            <motion.div
            ...
            <div className="mt-auto pt-2 relative">
              ...
              <motion.div whileTap={{ scale: 0.95 }} className="relative">
                <Button
                  size="lg"
                  onClick={handleBrew}
                  className="h-20 w-full rounded-full text-[1.5rem] tracking-wide font-extrabold shadow-[0_5px_20px_rgba(251,191,36,0.3)] border-b-[5px] active:border-b-0 active:translate-y-1.25 transition-all bg-linear-to-r from-amber-400 via-amber-500 to-amber-600 text-stone-950 border-amber-700"
                >
                  BUAT JAMU INI
                </Button>
              </motion.div>
        ...
        <motion.div 
          ...
          className="flex flex-col justify-center w-125 shrink-0"
        >
          <div className="bg-amber-500/5 border border-amber-500/20 rounded-[2.5rem] p-10 backdrop-blur-sm shadow-[inset_0_0_50px_rgba(251,191,36,0.03)]">
            ...
            <div className="relative flex flex-col items-center justify-center min-h-30">
